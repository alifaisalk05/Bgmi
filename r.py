...
bot = telebot.TeleBot('7711044348:AAFI0RRsJA5jwNuayFNsJdAH2WT95bODU2o')
...
MAX_ATTACK_TIME = 300  # Example: Default maximum attack time set to 239 seconds
...
def process_redeem_key(message):
    key = message.text.strip()
    if key in keys and keys[key]["status"] == "valid":
        duration = keys[key].get("duration", "1day")  # default 1 day if missing
        keys[key]["status"] = "redeemed"
        save_keys()
        ...
        if "hour" in duration:
            hours = int(duration.replace("hours", "").replace("hour", ""))
            expiration_date = now + datetime.timedelta(hours=hours)
        elif "day" in duration:
            days = int(duration.replace("days", "").replace("day", ""))
            expiration_date = now + datetime.timedelta(days=days)
        else:
            expiration_date = now + datetime.timedelta(days=1)  # fallback

        users[str(message.chat.id)] = expiration_date.strftime('%Y-%m-%d %H:%M:%S')
        save_users()
...
def process_genkey(call):
    ...
    # ✅ Save duration with key
    keys[key] = {"status": "valid", "duration": duration}
    save_keys()
...
def my_info(message):
    ...
    if user_id in admin_id:
        ...
        role = "User"
        expiration = users.get(user_id, "Expired")

        if expiration != "Expired":
            try:
                exp_date = datetime.datetime.strptime(expiration, "%Y-%m-%d %H:%M:%S")
                now = datetime.datetime.now()
                if exp_date > now:
                    delta = exp_date - now
                    days = delta.days
